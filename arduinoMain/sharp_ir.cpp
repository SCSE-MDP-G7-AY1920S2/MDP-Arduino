...
ZSharpIR::ZSharpIR(int ir_pin, const uint32_t model,
...
int ZSharpIR::getIrPin() const {
	return ir_pin_;
}

int ZSharpIR::getRaw() const
{
	int ir_val[kNbSample];
	for (int i = 0; i < kNbSample; i++) 
		ir_val[i] = analogRead(ir_pin_);
	sort(ir_val, kNbSample);

	return ir_val[kNbSample / 2];
}

int ZSharpIR::distance() const
{
  int ir_val = getRaw();
	return compute(ir_val);
}

int ZSharpIR::compute(int ir_val) const {
  if (ir_val <= -b_)
    return -1;
