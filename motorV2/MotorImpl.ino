...
// Ticks lookup table.
const int TICKS[15] = {273, 609, 904, 1183, 1480, 1790, 2090, 2390, 2705, 2980, 3275, 3590, 3855, 4130, 4430}; //282
const int TICKS_FAST[15] = {282, 599, 890, 1163, 1455, 1790, 2080, 2390, 2715, 2970, 3265, 3580, 3845, 4160, 4420}; //280
...
void turnLeft(int angle) {
  if (angle >= 90 && angle % 90 == 0)
    turnLeftTicks(angle, /*stepSize=*/90, TURN_TICKS_L_90, 
    TURN_FAST_SPEED, ShortTurnPID);
  else if (angle % 45 == 0)
    turnLeftTicks(angle, /*stepSize=*/45, TURN_TICKS_L_45, 
    TURN_NORMAL_SPEED, ShortTurnPID);
  else if (angle % 10 == 0)
    turnLeftTicks(angle, /*stepSize=*/10, TURN_TICKS_L_10, 
    TURN_NORMAL_SPEED, ShortTurnPID);
  else
    turnLeftTicks(angle, /*stepSize=*/1, TURN_TICKS_L_1, 
    TURN_SLOW_SPEED, ShortTurnPID);
}

void turnRight(int angle) {
  if (angle >= 90 && angle % 90 == 0)
    turnRightTicks(angle, /*stepSize=*/90, TURN_TICKS_R_90, 
    TURN_FAST_SPEED, ShortTurnPID);
  else if (angle % 45 == 0)
    turnRightTicks(angle, /*stepSize=*/45, TURN_TICKS_R_45, 
    TURN_NORMAL_SPEED, ShortTurnPID);
  else if (angle % 10 == 0)
    turnRightTicks(angle, /*stepSize=*/10, TURN_TICKS_R_10, 
    TURN_NORMAL_SPEED, ShortTurnPID);
  else
    turnRightTicks(angle, /*stepSize=*/1, TURN_TICKS_R_1, 
    TURN_SLOW_SPEED, ShortTurnPID);
}
...
void goForwardTicks(const int totalTicks, const int baseSpeed, 
  const FastPID& pid) {
  setTicks();
  startMotor();
  ...
  while (rightTick <= totalTicks || leftTick <= totalTicks) {
    ...
    if (now - lastTime >= SampleTime) {
      ...
      if (offset >= 1) {
        md.setSpeeds(currentSpeed + tickOffset, 
...
void goBackwardTicks(const int totalTicks, const int baseSpeed, 
  const FastPID& pid) {
  setTicks();
  startMotor();
  int currentSpeed = baseSpeed;
...
void turnLeftTicks(const int totalAngle, const int stepSize, 
	const int turnTicks, const int currentSpeed, const FastPID& pid) {
...
int cmToTicks(int* ticks, int cm) {
  int dist = (cm / 10) - 1;
  if (dist < 15)
    return ticks[dist];
  return 0;
